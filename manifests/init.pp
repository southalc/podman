...
class podman (
  ...
  Optional[Enum['absent', 'installed']]             $buildah_pkg_ensure       = undef,
  Optional[Enum['absent', 'installed']]             $podman_docker_pkg_ensure = 'installed',
  Optional[Pattern[/^(\d+\.){2}\d+$/, /absent/, /installed/]] $compose_pkg_ensure = undef,
  Optional[Enum['absent', 'installed']]             $machinectl_pkg_ensure    = 'installed',
